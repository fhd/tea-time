...
{
	private static final Command START_COMMAND =
			new Command("Start", Command.SCREEN, 0);
	private static final Command BACK_COMMAND =
			new Command("Back", Command.BACK, 1);
	private static final Command EDIT_COMMAND =
			new Command("Edit tea list", Command.SCREEN, 1);
	...
	protected void startApp() throws MIDletStateChangeException
	{
		...
		CommandListener cl = new CommandListener()
		{
			public void commandAction(Command c, Displayable d)
			{
				...
				if (teaList.equals(currentDisplay)
					...
					String teaLabel = teaList.getString(
							teaList.getSelectedIndex());
				...
				if (START_COMMAND.getLabel().equals(label)) {
					if (customTeaForm.equals(currentDisplay)) {
						...
						TeaTime.this.selectedTea = new Tea(
								CUSTOM_TEA.getName(),
		...
		this.drawTimeSecondsField = new TextField(
				"Seconds", "", 5, TextField.NUMERIC);
		this.customTeaForm.append(this.drawTimeSecondsField);
		this.drawTimeMinutesField = new TextField(
				"Minutes", "", 5, TextField.NUMERIC);
	...
	{	
		this.teaTimerTea.setLabel(this.selectedTea.getName());
		this.teaTimerTea.setText(" "
				+ String.valueOf(this.selectedTea.getDrawTime())
				+ " seconds");
		...
		this.timer.scheduleAtFixedRate(new TimerTask()
		{
			...
			public void run()
			{
				...
				if (secondsLeft <= 0) {
					...
					display.setCurrent(alert);
					// TODO: Do something for phones without vibration support.
					try {
